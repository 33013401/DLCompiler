...
build_wafer_deps=OFF
build_dev=OFF
build_flagtree_wafer_deps=OFF
...
show_help() {
    ...
    echo "support mode:"
    echo "  build_flagtree_wafer_deps"
    echo "  build_wafer_deps"
    echo "  build_dev"
    echo ""
    echo "tx81fw url:"
...
MODE=$1
case "$MODE" in
    build_tx8_deps) MODE=build_wafer_deps ;;
    build_flagtree_tx8_deps) MODE=build_flagtree_wafer_deps ;;
esac
wafer_firmware_url=http://172.50.1.66:8082/artifactory/tx8-generic-dev/tx81fw/tx81fw_202512041135_b731cf.tar.gz
if [ $# -ge 2 ]; then
	wafer_firmware_url=$2
fi
echo "tx81fw: "$wafer_firmware_url
...
if [ "x$MODE" == "xbuild_flagtree_wafer_deps" ] || [ "x$MODE" == "xbuild_wafer_deps" ] || [ "x$MODE" == "xbuild_dev" ]; then
    download_dir=$WORKSPACE/download
    ########################################################################################
    wafer_firmware_dir=$download_dir/triton-tx8fw
    download_and_extract $wafer_firmware_url \
        "$wafer_firmware_dir" "$download_dir" "tx8fw"

    ########################################################################################
    xuantie_sdk_dir=$download_dir/tx8fw-xuantie-sdk
    clone_and_checkout "git@gitlab.wafer.com:tx8_developers/tx8fw-xuantie-sdk.git" \
    ...
    wafer_profiler_dir=$download_dir/tx_profiler
    download_and_extract $profile_url \
        "$wafer_profiler_dir" "$download_dir" "tx_profiler"

    ########################################################################################
    wafer_deps_dir=$WORKSPACE/tx8_deps
    if [ -d $wafer_deps_dir ]; then
        rm -rf $wafer_deps_dir
    fi
    mkdir $wafer_deps_dir

    load_copy_cfg
    echo -e $PROJECT_INFO > $wafer_deps_dir/version.txt

    copy_files $MODE $wafer_deps_dir

    pushd $WORKSPACE
        current_time=$(date +%Y%m%d_%H%M%S)
        tar_flag="release"
        if [ "x$MODE" == "xbuild_wafer_deps" ] || [ "x$MODE" == "xbuild_dev" ]; then
            tar_flag="dev"
        fi
    ...
    # # clone_and_checkout "git@gitlab.wafer.com:triton-based-projects/llvm-project.git" "$WORKSPACE/llvm-project-for-ztc" "branch" "ztc"
    # clone_and_checkout "git@gitlab.wafer.com:triton-based-projects/llvm-project.git" "$WORKSPACE/llvm-project" "commit" "a66376b0dc3b2ea8a84fda26faca287980986f78"
