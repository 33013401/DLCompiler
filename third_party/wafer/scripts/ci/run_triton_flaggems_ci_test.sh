...
#git clone "http://gitlab.wafer.com/triton-based-projects/flaggems.git" -b board-test-base

##2.创建容器(CI负责)
#docker run -d --name jenkins_tc_triton_ci --network=host --ipc=host --privileged -v /dev:/dev -v /tmp:/tmp -v /lib/modules:/lib/modules -v /sys:/sys -v /login_home/:/login_home/ -w /login_home/jenkins_tc/ hub.wafer.com/tx8/ubuntu/v5.5.0.1030:wafer_release
...
#cd triton
#bash triton/third_party/wafer/scripts/ci/run_triton_flaggems_ci_test.sh 0 0 0 ci_ops 1
...
if [ ! -d "./llvm-a66376b0-ubuntu-x64" ]; then
	wget https://toolchain-jfrog.wafer.xyz:443/artifactory/tx8-generic-dev/triton/tools/llvm-a66376b0-ubuntu-x64.tar.gz
	if [ $? -eq 0 ]; then
...
if [ ! -d "./offline_pkgs" ]; then
	wget https://toolchain-jfrog.wafer.xyz:443/artifactory/tx8-generic-dev/triton/offline_pkgs/offline_pkgs_v5.3.0.tar.gz
	if [ $? -eq 0 ]; then
...
if [ $skip_install -ne 1 ]; then
	...
	python3 --version
	bash third_party/wafer/scripts/tools/offline_python_deps.sh -i -r python/requirements.txt -d ../offline_pkgs
	if [ $? -eq 0 ]; then
	...
	bash third_party/wafer/scripts/tools/offline_python_deps.sh -i -r third_party/wafer/scripts/requirements_ts.txt -d ../offline_pkgs
	if [ $? -eq 0 ]; then
...
if [ $skip_build -ne 1 ]; then
	bash ./third_party/wafer/scripts/build_wafer.sh
	if [ $? -eq 0 ]; then
