...
public:
  ...
  LogicalResult matchReduction(ReduceOp op) const {
    ...
    rewriter.replaceOpWithNewOp<linalg::GenericOp>(
        ...
        [&](OpBuilder &builder, Location loc, ValueRange regionArgs) {
          ...
          auto *scalarOp = builder.create(
              ...
              regionArgs.take_front(op->getNumOperands()), resultTypes,
              op->getAttrs());
          builder.create<linalg::YieldOp>(loc, scalarOp->getResults());
        });
    return success();
...
class StorePtrToLinalgConverter : public OpConversionPattern<triton::StoreOp> {
  ...
  matchAndRewrite(triton::StoreOp op, OpAdaptor adaptor,
    ...
    SmallVector<utils::IteratorType> iteratorTypes(
        rank, utils::IteratorType::parallel);
    rewriter.replaceOpWithNewOp<linalg::GenericOp>(
        op, op->getResultTypes(), op->getOperands(), ValueRange{}, indexingMaps,
        iteratorTypes,
        [&](OpBuilder &builder, Location loc, ValueRange regionArgs) {
          auto resultTypes = llvm::map_to_vector(
              op->getResultTypes(), [](Type type) {
