...
@triton.jit
def to_buffer_to_tensor_kernel(x_ptr, y_ptr, out_ptr, M, N, P, Q, BM: tl.constexpr, BN: tl.constexpr, BP: tl.constexpr,
                               BQ: tl.constexpr):
    ...
    buf_x = tle.dsa.to_buffer(x, tle.dsa.spm)
    buf_y = tle.dsa.to_buffer(y, tle.dsa.spm)
    ...
    buf_z = tle.dsa.to_buffer(z, tle.dsa.spm)
...
class TestTLEDsaBridge:
    ...
    def test_bridge(self, shape, block):
        ...
        m, n, p, q = shape
        bm, bn, bp, bq = block
        a = torch.randn(*shape, device="cpu", dtype=torch.float32)
        b = torch.randn(*shape, device="cpu", dtype=torch.float32)
