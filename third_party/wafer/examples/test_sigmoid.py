...
def test_sigmoid(size, dtype, device="cpu"):
    # Generate random input data
    x = torch.randn(size, device="cpu", dtype=dtype)

    # Call the Triton kernel
    output = sigmoid_triton(x)
...
@benchmark.measure()
def benchmark_sigmoid_triton(size, dtype, provider):
    if provider != "triton":
...
if __name__ == "__main__":
    for size in [i**2 for i in range(22, 25, 1)]:
        benchmark_sigmoid_triton(size, torch.float32, "triton")
