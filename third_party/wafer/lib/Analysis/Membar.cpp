...
#include "wafer/Dialect/IR/WaferOps.h"
...
static void collectWaferAccesses(Operation *op, SmallVector<Value> &reads,
  ...
  if (auto iface = dyn_cast<MemoryEffectOpInterface>(op)) {
    ...
    // Many Wafer ops annotate destination operands with MemWrite but leave
...
void MembarAnalysis::visitTerminator(Operation *op,
  ...
  if (op->hasTrait<OpTrait::ReturnLike>())
    return;
  llvm_unreachable("Unknown terminator encountered in Wafer membar analysis");
}

void MembarAnalysis::insertBarrier(Operation *op, OpBuilder *builder) {
  OpBuilder::InsertionGuard g(*builder);
  builder->create<wafer::BarrierOp>(op->getLoc());
}

void MembarAnalysis::update(Operation *op, BlockInfo *blockInfo,
                            FuncBlockInfoMapT *funcBlockInfoMap,
                            OpBuilder *builder) {
  if (isa<wafer::BarrierOp>(op)) {
    blockInfo->sync();
    return;
  }
  ...
  if (isa<triton::CallOp>(op)) {
    ...
    if (isTxDialect(op)) {
      collectWaferAccesses(op, reads, writes);
