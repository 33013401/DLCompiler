...
public:
  ...
  void runOnOperation() override {
    ...
    auto addFunctionToSections = [&](LLVM::LLVMFuncOp funcOp,
                                     StringRef funcName) {
      std::string symbolName = funcName.str();
      Type arrayType =
          LLVM::LLVMArrayType::get(ctx, i8Type, symbolName.size() + 1);
      ...
      RankedTensorType tensorType =
          RankedTensorType::get({static_cast<int64_t>(bytes.size())}, i8Type);
      DenseElementsAttr nameAttr =
          DenseElementsAttr::get(tensorType, ArrayRef<char>(bytes));
      ...
      LLVM::GlobalOp nameVar = builder.create<LLVM::GlobalOp>(
          module.getLoc(), arrayType, /*isConstant=*/true, LLVM::Linkage::Weak,
          nameVarName, nameAttr);
      ...
      LLVM::GlobalOp symtabVar = builder.create<LLVM::GlobalOp>(
          module.getLoc(), symtabType, /*isConstant=*/true,
          LLVM::Linkage::External, symtabVarName, Attribute());
      ...
      Value funcAddr = builder.create<LLVM::AddressOfOp>(
          module.getLoc(), ptrType, funcOp.getSymNameAttr());
      Value nameAddr = builder.create<LLVM::AddressOfOp>(
          module.getLoc(), ptrType, nameVar.getSymNameAttr());

      Value structVal =
          builder.create<LLVM::UndefOp>(module.getLoc(), symtabType);
      structVal = builder.create<LLVM::InsertValueOp>(
          module.getLoc(), symtabType, structVal, funcAddr,
          builder.getDenseI64ArrayAttr({0}));
      structVal = builder.create<LLVM::InsertValueOp>(
          module.getLoc(), symtabType, structVal, nameAddr,
    ...
    LLVM::LLVMFunctionType funcType = LLVM::LLVMFunctionType::get(
        voidType, {voidPtrType}, /*isVarArg=*/false);
    ...
    auto getOrCreateFunc = [&](StringRef name) -> LLVM::LLVMFuncOp {
      ...
      auto func = builder.create<LLVM::LLVMFuncOp>(
          module.getLoc(), name, funcType, LLVM::Linkage::External,
...
triton::createExportKernelSymbolsPass() {
