...
#include "wafer-tx81/Conversion/Tx81MemrefToLLVM/Tx81MemrefToLLVM.h"
...
#include "wafer-tx81/Conversion/Tx81MemrefToLLVM/Passes.h.inc"
...
struct TsmMemRefAllocOpLowering
  ...
  allocateBufferFromSPM(ConversionPatternRewriter &rewriter, Location loc,
    ...
    spmAddr = rewriter.create<LLVM::PtrToIntOp>(allocOp.getLoc(),
                                                rewriter.getI64Type(), spmAddr);
    spmAddr = rewriter.create<LLVM::AddOp>(
      allocOp.getLoc(), rewriter.getI64Type(), spmAddr, spmOffsetOp);

    spmAddr = rewriter.create<LLVM::IntToPtrOp>(allocOp.getLoc(),
                          elementPtrType, spmAddr);
  ...
  matchAndRewrite(memref::AllocOp op, OpAdaptor adaptor,
    ...
    Value sizeBytes;
    getMemRefDescriptorSizes(op.getLoc(), memRefType, adaptor.getOperands(),
                             rewriter, sizes, strides, sizeBytes);

    auto [allocatedPtr, alignedPtr] =
        allocateBufferFromSPM(rewriter, op.getLoc(), op);
    if (!allocatedPtr)
      return failure();

    auto descriptor = createMemRefDescriptor(
        op.getLoc(), memRefType, allocatedPtr, alignedPtr, sizes, strides,
        rewriter);
    rewriter.replaceOp(op, ValueRange{descriptor});
    return success();
  }
};
  ...
  matchAndRewrite(MemrefOp op, OpAdaptor adaptor,
    ...
    if (isMaskEle) {
      ...
      dataPtr = ConvertToLLVMPattern::getStridedElementPtr(
          rewriter, op.getLoc(), type, adaptor.getMemref(),
          adaptor.getIndices());
    }
...
public:
  LogicalResult matchCast(memref::CastOp memRefCastOp) const {
  ...
  matchAndRewrite(memref::CastOp memRefCastOp, OpAdaptor adaptor,
                  ConversionPatternRewriter &rewriter) const override {
    if (failed(matchCast(memRefCastOp)))
      return failure();
