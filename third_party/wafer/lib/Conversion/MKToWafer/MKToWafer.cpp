...
class MKToWaferTypeConverter : public TypeConverter {
public:
  MKToWaferTypeConverter() {
...
// This maps MLIR types to Wafer format codes
...
Data_Format insertConvertTypeOp(Value valuePtr, MemRefType valueType,
  ...
  case 16: { // 16 bit integer
    rewriter.create<wafer::INT16ToFP16Op>(loc, rewriter.getI64Type(), valuePtr,
                                       valuePtr, elemCount);
    fmt = Fmt_FP16;
    break;
  }
  case 32: { // 32 bit integer
    rewriter.create<wafer::INT32ToFP32Op>(loc, rewriter.getI64Type(), valuePtr,
                                       valuePtr, elemCount,
...
Value insertRestoreTypeOp(Value valuePtr, MemRefType valueType, Value elemCount,
  ...
  switch (bitWidth) {
  case 16: { // 16 bit integer
    newValue = rewriter.create<wafer::FP16ToINT16Op>(
        loc, rewriter.getI64Type(), valuePtr, valuePtr, elemCount,
        rewriter.getI16IntegerAttr(roundMode));
    break;
  }
  case 32: { // 32 bit integer
    newValue = rewriter.create<wafer::FP32ToINT32Op>(
        loc, rewriter.getI64Type(), valuePtr, valuePtr, elemCount,
        rewriter.getI16IntegerAttr(roundMode));
    break;
  }
...
public:
  ...
  matchAndRewrite(memref::CopyOp op, OpAdaptor adaptor,
    ...
    if (srcMemrefType.areTrailingDimsContiguous(rank) &&
      ...
      if (isDstSPM)
        rewriter.replaceOpWithNewOp<wafer::Rdma1dOp>(op, dstPtr, srcPtr, elemCount,
                                                  fmt);
      else
        rewriter.replaceOpWithNewOp<wafer::Wdma1dOp>(op, dstPtr, srcPtr, elemCount,
                                                  fmt);
      return success();
    }
    ...
    if (isDstSPM) {
      auto rdmaOp = rewriter.create<wafer::RdmaOp>(
      ...
      auto wdmaOp = rewriter.create<wafer::WdmaOp>(
          op.getLoc(), rewriter.getI64Type(), srcPtr, dstPtr,
          srcSizes,                              // src shape
          srcStrides,                            // src stride
          dstSizes,                              // dst shape
...
public:
  ...
  matchAndRewrite(linalg::FillOp op, OpAdaptor adaptor,
    ...
    auto resultOp = rewriter.create<wafer::MemsetOp>(
...
public:
  ...
  bool convertToGatherScatter(linalg::TransposeOp op, OpAdaptor adaptor,
    ...
    rewriter.create<wafer::GatherScatter>(
  ...
  matchAndRewrite(linalg::TransposeOp op, OpAdaptor adaptor,
    ...
    if (perm == ArrayRef<int64_t>({0, 2, 3, 1})) {
      return transposeChannel<wafer::Nchw2nhwc>(op, adaptor, rewriter);
    }

    if (perm == ArrayRef<int64_t>({0, 3, 1, 2})) {
      return transposeChannel<wafer::Nhwc2nchw>(op, adaptor, rewriter);
    }
    ...
    // TODO:  Convert higher rank to wafer.
    return convertLinalgOpToLoops(op, rewriter);
  }
};
...
public:
  ...
  matchAndRewrite(linalg::ReciprocalOp op, OpAdaptor adaptor,
    ...
    rewriter.create<wafer::RecipVVOp>(loc, rewriter.getI64Type(), inputPtr,
                                   outputPtr, elemCount,
...
//===----------------------------------------------------------------------===//
// mk.dot to wafer.gemm Conversion Pattern
//===----------------------------------------------------------------------===//

class MKDotToWaferGemmOpConversion : public OpConversionPattern<mk::DotOp> {
public:
  ...
  matchAndRewrite(mk::DotOp op, OpAdaptor adaptor,
    ...
    rewriter.create<wafer::GemmOp>(
        ...
        rewriter.getI32IntegerAttr(0), // relu_mode: no activation.
...
class MKDequantOpConversionPattern : public OpConversionPattern<mk::DequantOp> {
  ...
  matchAndRewrite(mk::DequantOp op, OpAdaptor adaptor,
    ...
    if (elemType.isBF16())
      rewriter.create<wafer::MXFPScaleBF16Op>(loc, TypeRange{}, inputPtr, scalePtr,
                                           outputPtr, elemCount);
    else
      rewriter.create<wafer::MXFPScaleFP16Op>(loc, TypeRange{}, inputPtr, scalePtr,
                                           outputPtr, elemCount);
    rewriter.eraseOp(op);
    return success();
  }
...
class MKSigmoidToWaferSigmoidOpConversion
...
public:
  ...
  matchAndRewrite(mlir::mk::SigmoidOp op, OpAdaptor adaptor,
    ...
    rewriter.create<wafer::Sigmoid>(loc, rewriter.getI64Type(), input, dst,
...
struct MKGeluToWaferGeluOpConversion
  ...
  matchAndRewrite(mlir::mk::GeluOp op, OpAdaptor adaptor,
    ...
    switch (geluMode) {
    case GeluMode::None: {
      rewriter.create<wafer::GeluNone>(loc, rewriter.getI64Type(), input, dst,
                                    elemCount,
                                    rewriter.getI16IntegerAttr(srcFmt));
      break;
    }
    case GeluMode::Tanh: {
      ...
      rewriter.create<wafer::GeluTanh>(loc, rewriter.getI64Type(), input, immAddr,
                                    dst, elemCount,
                                    rewriter.getI16IntegerAttr(srcFmt));
      break;
    }
...
public:
  ...
  matchAndRewrite(mlir::mk::Bit2FpOp op, OpAdaptor adaptor,
    ...
    rewriter.create<wafer::Bit2FpOp>(loc, rewriter.getI64Type(), inputPtr,
                                  outputPtr, elemCount,
                                  rewriter.getI16IntegerAttr(srcFmt));
    rewriter.eraseOp(op);
...
public:
  ...
  matchAndRewrite(mlir::mk::MaskMoveOp op, OpAdaptor adaptor,
    ...
    rewriter.create<wafer::MaskMoveOp>(loc, rewriter.getI64Type(), inputPtr,
...
struct ElementwiseConversion : public OpConversionPattern<linalg::GenericOp> {
  ...
  LogicalResult convertIsNaNOp(linalg::GenericOp op, OpAdaptor adapter,
    ...
    rewriter.create<wafer::BoolUnEqualVV>(loc,                   // loc
  ...
  LogicalResult FmaConvertOp(linalg::GenericOp op, OpAdaptor adaptor,
    ...
    auto mulResult = rewriter.create<wafer::MulVVOp>(
        loc, rewriter.getI64Type(), input0, input1, output, elemCount,
        rewriter.getI16IntegerAttr(0), // Round mode
        rewriter.getI16IntegerAttr(getFormatCode(inputType)));
    auto addResult = rewriter.create<wafer::AddVVOp>(
  ...
  LogicalResult convertDivIntOp(linalg::GenericOp op, OpAdaptor adaptor,
    ...
    rewriter.create<wafer::DivVVOp>(loc, rewriter.getI64Type(), input0, input1,
                                 output, elemCount,
                                 rewriter.getI16IntegerAttr(0), // Round mode
                                 rewriter.getI16IntegerAttr(srcFmt));
  ...
  convertRoundOp(linalg::GenericOp op, OpAdaptor adaptor,
    ...
    auto fpToInt = rewriter.create<wafer::FP32ToINT32Op>(
        loc, rewriter.getI64Type(), input, output, elemCount,
        rewriter.getI16IntegerAttr(roundMode)); // Round mode
    auto intToFp = rewriter.create<wafer::INT32ToFP32Op>(
  ...
  matchAndRewrite(linalg::GenericOp op, OpAdaptor adaptor,
    ...
    return llvm::TypeSwitch<Operation *, LogicalResult>(elemWiseOp)
        .Case<math::TanhOp>([&](auto elemWiseOp) {
          return convertUnaryOp<wafer::Tanh>(op, adaptor, rewriter);
        })
        .Case<arith::NegFOp>([&](auto elemWiseOp) {
          return convertUnaryOp<wafer::NegVVOp>(op, adaptor, rewriter);
        })
        .Case<math::IsNaNOp>([&](auto elemWiseOp) {
          return convertIsNaNOp(op, adaptor, rewriter);
        })
        .Case<arith::AddFOp>([&](auto elemWiseOp) {
          return convertBinaryOp<wafer::AddVVOp>(op, adaptor, rewriter);
        })
        .Case<arith::SubFOp>([&](auto elemWiseOp) {
          return convertBinaryOp<wafer::SubVVOp>(op, adaptor, rewriter);
        })
        .Case<arith::MulFOp>([&](auto elemWiseOp) {
          return convertBinaryOp<wafer::MulVVOp>(op, adaptor, rewriter);
        })
        ...
        .Case<arith::MaximumFOp>([&](auto elemWiseOp) {
          return convertBinaryOp<wafer::MaxVVOp>(op, adaptor, rewriter);
        })
        .Case<arith::MinimumFOp>([&](auto elemWiseOp) {
          return convertBinaryOp<wafer::MinVVOp>(op, adaptor, rewriter);
        })
        .Case<arith::AndIOp>([&](auto elemWiseOp) {
          return convertBoolBinaryLogicOp<wafer::BoolAndV>(op, adaptor, rewriter);
        })
        .Case<arith::OrIOp>([&](auto elemWiseOp) {
          return convertBoolBinaryLogicOp<wafer::BoolOrV>(op, adaptor, rewriter);
        })
        .Case<arith::XOrIOp>([&](auto elemWiseOp) {
          return convertBoolBinaryLogicOp<wafer::BoolXorV>(op, adaptor, rewriter);
        })
        .Case<math::AbsFOp>([&](auto elemWiseOp) {
          return convertUnaryOp<wafer::AbsVVOp>(op, adaptor, rewriter);
        })
        ...
        .Case<math::SqrtOp>([&](auto elemWiseOp) {
          return convertUnaryOp<wafer::SqrtVVOp>(op, adaptor, rewriter);
        })
        .Case<math::RsqrtOp>([&](auto elemWiseOp) {
          return convertUnaryOp<wafer::RsqrtVVOp>(op, adaptor, rewriter);
        })
        .Case<math::LogOp>([&](auto elemWiseOp) {
          return convertUnaryOp<wafer::LnOp>(op, adaptor, rewriter);
        })
        .Case<math::Log2Op>([&](auto elemWiseOp) {
          return convertUnaryOp<wafer::Log2Op>(op, adaptor, rewriter);
        })
        .Case<math::ExpOp>([&](auto elemWiseOp) {
          return convertUnaryOp<wafer::ExpOp>(op, adaptor, rewriter);
        })
        .Case<math::Exp2Op>([&](auto elemWiseOp) {
          return convertUnaryOp<wafer::Pow2Op>(op, adaptor, rewriter);
        })
        .Case<math::SinOp>([&](auto elemWiseOp) {
          return convertUnaryOp<wafer::SinOp>(op, adaptor, rewriter);
        })
        .Case<math::CosOp>([&](auto elemWiseOp) {
          return convertUnaryOp<wafer::CosOp>(op, adaptor, rewriter);
        })
        .Case<arith::ExtFOp>([&](auto elemWiseOp) {
          ...
          if (inputType.isF16() && targetType.isF32())
            return NormalConvertOp<wafer::FP16ToFP32Op>(op, adaptor, rewriter);
          else if (inputType.isBF16() && targetType.isF32())
            return NormalConvertOp<wafer::BF16ToFP32Op>(op, adaptor, rewriter);

          else if (isa<Float8E5M2Type>(inputType) && targetType.isBF16())
            return NormalConvertOp<wafer::FP8E5M2ToBF16Op>(op, adaptor, rewriter);
          else if (isa<Float8E4M3Type>(inputType) && targetType.isBF16())
            return NormalConvertOp<wafer::FP8E4M3ToBF16Op>(op, adaptor, rewriter);
          else if (isa<Float8E4M3FNType>(inputType) && targetType.isBF16())
            return NormalConvertOp<wafer::FP8E4M3FNToBF16Op>(op, adaptor,
                                                          rewriter);
          else if (isa<Float4E2M1FNType>(inputType) && targetType.isBF16())
            return NormalConvertOp<wafer::FP4E2M1ToBF16Op>(op, adaptor, rewriter);

          else if (isa<Float8E5M2Type>(inputType) && targetType.isF16())
            return NormalConvertOp<wafer::FP8E5M2ToFP16Op>(op, adaptor, rewriter);
          else if (isa<Float8E4M3Type>(inputType) && targetType.isF16())
            return NormalConvertOp<wafer::FP8E4M3ToFP16Op>(op, adaptor, rewriter);
          else if (isa<Float8E4M3FNType>(inputType) && targetType.isF16())
            return NormalConvertOp<wafer::FP8E4M3FNToFP16Op>(op, adaptor,
                                                          rewriter);
          else if (isa<Float4E2M1FNType>(inputType) && targetType.isF16())
            return NormalConvertOp<wafer::FP4E2M1ToFP16Op>(op, adaptor, rewriter);
          else
            return rewriter.notifyMatchFailure(
        ...
        .Case<arith::SIToFPOp>([&](auto elemWiseOp) {
          ...
          if (inputType.isInteger(8) && outputType.isF32()) {
            return ZeroPointConvertOp<wafer::INT8ToFP32Op>(op, adaptor, rewriter);
          } else if (inputType.isInteger(8) && outputType.isF16()) {
            return ZeroPointConvertOp<wafer::INT8ToFP16Op>(op, adaptor, rewriter);
          } else if (inputType.isInteger(16) && outputType.isF32()) {
            return RoundConvertOp<wafer::INT16ToFP32Op>(op, adaptor, rewriter);
          } else if (inputType.isInteger(16) && outputType.isF16()) {
            return NormalConvertOp<wafer::INT16ToFP16Op>(op, adaptor, rewriter);
          } else if (inputType.isInteger(32) && outputType.isF16()) {
            return RoundConvertOp<wafer::INT32ToFP16Op>(op, adaptor, rewriter);
          } else if (inputType.isInteger(32) && outputType.isF32()) {
            return RoundConvertOp<wafer::INT32ToFP32Op>(op, adaptor, rewriter);
          } else {
            return rewriter.notifyMatchFailure(
                op, "Unsupported input/output type combination for integer to "
                    "FP conversion");
        ...
        .Case<arith::FPToSIOp>([&](auto elemWiseOp) {
          ...
          if (inputType.isF16() && outputType.isInteger(8)) {
            return RoundConvertOp<wafer::FP16ToINT8Op>(op, adaptor, rewriter,
                                                    RND_MODE::RND_ZERO);
          } else if (inputType.isF16() && outputType.isInteger(16)) {
            return RoundConvertOp<wafer::FP16ToINT16Op>(op, adaptor, rewriter,
                                                     RND_MODE::RND_ZERO);
          } else if (inputType.isF16() && outputType.isInteger(32)) {
            return RoundConvertOp<wafer::FP16ToINT32Op>(op, adaptor, rewriter,
                                                     RND_MODE::RND_ZERO);
          } else if (inputType.isF32() && outputType.isInteger(8)) {
            return RoundConvertOp<wafer::FP32ToINT8Op>(op, adaptor, rewriter,
                                                    RND_MODE::RND_ZERO);
          } else if (inputType.isF32() && outputType.isInteger(16)) {
            return RoundConvertOp<wafer::FP32ToINT16Op>(op, adaptor, rewriter,
                                                     RND_MODE::RND_ZERO);
          } else if (inputType.isF32() && outputType.isInteger(32)) {
            return RoundConvertOp<wafer::FP32ToINT32Op>(op, adaptor, rewriter,
                                                     RND_MODE::RND_ZERO);
          } else {
            return rewriter.notifyMatchFailure(
        ...
        .Case<arith::CmpFOp>([&](auto elemWiseOp) {
          ...
          case arith::CmpFPredicate::UEQ:
            return BoolRelationVVOp<wafer::BoolEqualVV>(op, adaptor, rewriter);
          case arith::CmpFPredicate::ONE:
          case arith::CmpFPredicate::UNE:
            return BoolRelationVVOp<wafer::BoolUnEqualVV>(op, adaptor, rewriter);
          case arith::CmpFPredicate::OGE:
          case arith::CmpFPredicate::UGE:
            return BoolRelationVVOp<wafer::BoolGreaterEqualVV>(op, adaptor,
                                                            rewriter);
          case arith::CmpFPredicate::OGT:
          case arith::CmpFPredicate::UGT:
            return BoolRelationVVOp<wafer::BoolGreaterVV>(op, adaptor, rewriter);
          case arith::CmpFPredicate::OLE:
          case arith::CmpFPredicate::ULE:
            return BoolRelationVVOp<wafer::BoolLessEqualVV>(op, adaptor, rewriter);
          case arith::CmpFPredicate::OLT:
          case arith::CmpFPredicate::ULT:
            return BoolRelationVVOp<wafer::BoolLessThenVV>(op, adaptor, rewriter);
          default:
        ...
        .Case<arith::TruncFOp>([&](auto elemWiseOp) {
          ...
          if (resultType.isF16())
            return RoundConvertOp<wafer::FP32ToFP16Op>(op, adaptor, rewriter);
          else if (resultType.isBF16())
            return RoundConvertOp<wafer::FP32ToBF16Op>(op, adaptor, rewriter);
          else
            return rewriter.notifyMatchFailure(
...
public:
  ...
  matchAndRewrite(linalg::ReduceOp op, OpAdaptor adaptor,
    ...
    // If there is only one reduction operation, try to convert it to Wafer
    // reduce op.
...
struct RemoteLoadConversion : public OpConversionPattern<mk::RemoteLoadOp> {
  ...
  matchAndRewrite(mk::RemoteLoadOp op, OpAdaptor adaptor,
    ...
    // Create wafer.remote_load operation.
    rewriter.create<wafer::RemoteLoadOp>(
        loc,
    ...
    // mk.remote_load has results; wafer.remote_load is void. Replace results with
...
struct RemoteStoreConversion : public OpConversionPattern<mk::RemoteStoreOp> {
  ...
  matchAndRewrite(mk::RemoteStoreOp op, OpAdaptor adaptor,
    ...
    if (dstAddrVal.getType().isInteger(64)) {
      ...
      return rewriter.notifyMatchFailure(
          op, "mk.remote_store dst_addr must be i64 or memref at MKToWafer");
    }

    // Create wafer.remote_store operation directly with the destination address.
    rewriter.create<wafer::RemoteStoreOp>(
        loc,
        adaptor.getOperands()[0], // remote_chip_id_x
        adaptor.getOperands()[1], // remote_chip_id_y
        adaptor.getOperands()[2], // remote_die_id
...
struct AtomicRMWOpConversion : public OpConversionPattern<mk::AtomicRMWOp> {
  ...
  matchAndRewrite(mk::AtomicRMWOp op, OpAdaptor adaptor,
    ...
    assert(0 && "Now, wafer backend only support memref type. Don't "
...
//===----------------------------------------------------------------------===//
// Legalize magic kernel operations to be convertible to Wafer operations
// patterns
//===----------------------------------------------------------------------===//
namespace {
...
struct MKRandGenOpConversion : public OpConversionPattern<mk::RandGenOp> {
  ...
  matchAndRewrite(mk::RandGenOp op, OpAdaptor adaptor,
    ...
    rewriter.replaceOpWithNewOp<wafer::RandGenOp>(
...
void mlir::triton::populateMKToWaferCanonicalizationPatterns(
    RewritePatternSet &patterns) {
  // Backend op canonicalization
  patterns.add<LinalgCopyRewrite>(patterns.getContext());
}

void mlir::triton::populateMKToWaferConversionPatterns(
    RewritePatternSet &patterns) {

  MKToWaferTypeConverter typeConverter;
  ...
  // NOTE: Only convert ops that have been legalized to be convertible to Wafer
  // ops.
  // Convert only float type input/output ops to Wafer ops except some special
  ...
  patterns.add<MemoryCopyConvertPattern,
               ...
               MKReduceOpConversion<mk::ReduceMaxOp, wafer::ReduceMaxOp>,
               MKReduceOpConversion<mk::ReduceMinOp, wafer::ReduceMinOp>,
               MKReduceOpConversion<mk::ReduceSumOp, wafer::ReduceSumOp>,
               TransposeOpConversion,
               ReciprocalOpConversionPattern,
               LinalgFillOpConversion,
               MKDotToWaferGemmOpConversion,
               MKDequantOpConversionPattern,
               MKSigmoidToWaferSigmoidOpConversion,
               MKGeluToWaferGeluOpConversion,
               MKArgMinMaxConversionPattern<mk::ArgMinOp, wafer::ArgMinOp>,
               MKArgMinMaxConversionPattern<mk::ArgMaxOp, wafer::ArgMaxOp>,
               MKMaskMoveOpConversionPattern,
               MKBit2FPOpConversionPattern,
               MKRelationVSOpConversionPattern<mk::BoolEqualVS,wafer::BoolEqualVS>,
               MKRelationVSOpConversionPattern<mk::EqualVS,wafer::EqualVS>,
               MKRelationVSOpConversionPattern<mk::LessThenVS, wafer::LessThenVS>,
               MKRelationVVOpConversionPattern<mk::UnEqualVV, wafer::UnEqualVV>,
               MKArithVSOpConversionPattern<mk::AddVS, wafer::AddVSOp>,
               MKArithVSOpConversionPattern<mk::SubVS, wafer::SubVSOp>,
               MKArithVSOpConversionPattern<mk::MulVS, wafer::MulVSOp>,
               MKRelationVVOpConversionPattern<mk::EqualVV,wafer::EqualVV>,
               GatherConvertPattern,
               BarrierConversion<mk::BarrierOp, wafer::BarrierOp>,
               BarrierConversion<mk::AtomicBarrierInOp,wafer::AtomicBarrierInOp>,
               BarrierConversion<mk::AtomicBarrierOutOp,wafer::AtomicBarrierOutOp>,
               PrintConversion,
