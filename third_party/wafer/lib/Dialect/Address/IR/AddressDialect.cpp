...
void CastIntOp::build(OpBuilder &odsBuilder, OperationState &odsState,
                      Value input, Type resultTy) {
  if (!resultTy)
    resultTy = isa<AddressType>(input.getType())
                   ? cast<Type>(odsBuilder.getIndexType())
                   : cast<Type>(odsBuilder.getType<AddressType>());
  build(odsBuilder, odsState, resultTy, input);
}
