//===----------- InsertBarrierPass.cpp - Wafer Barrier Insertion --------===//
//
...
//   `wafer::BarrierOp` only when needed.
// - Add a minimal DDR hazard check for WDMA->RDMA (and CPU touching DDR after
//   WDMA) to avoid stale reads.
//
// `wafer::BarrierOp` is later lowered in WaferToLLVM to `__Barrier` (equivalent to
...
#define DEBUG_TYPE "wafer-insert-barrier"
...
static bool isTxDialect(Operation *op) {
  ...
  return d && d->getNamespace() == "wafer";
}

static bool isExplicitBarrier(Operation *op) {
  return isa<wafer::BarrierOp, wafer::AtomicBarrierInOp, wafer::AtomicBarrierOutOp>(op);
}
...
static bool txBarrierBetweenLhsAndRhs(Operation *lhsOp, Operation *rhsOp) {
  ...
  funcOp.getOperation()->walk<WalkOrder::PreOrder>([&](Operation *op) {
    ...
    if (op == lhsOp)
      seenLhs = true;
    if (seenLhs &&
        (isa<wafer::BarrierOp, wafer::AtomicBarrierInOp, wafer::AtomicBarrierOutOp>(op)))
...
static void insertLoopCarriedSpmBarriers(ModuleOp mod) {
  mod.walk([&](scf::ForOp forOp) {
    ...
    if (Operation *prev = yield->getPrevNode();
        prev && isa<wafer::BarrierOp>(prev))
      return;
    OpBuilder b(yield);
    b.create<wafer::BarrierOp>(yield->getLoc());
  });
}
...
static void hoistTxBarriersFromScfForLoops(ModuleOp mod) {
  mod.walk<WalkOrder::PostOrder>([&](scf::ForOp forOp) {
    SmallVector<wafer::BarrierOp, 8> barriers;
    forOp.getBody()->walk([&](wafer::BarrierOp b) { barriers.push_back(b); });
    if (barriers.empty())
      return;

    SmallVector<wafer::BarrierOp, 8> toHoist;
    for (wafer::BarrierOp br : barriers) {
    ...
    b.create<wafer::BarrierOp>(forOp.getLoc());
    for (wafer::BarrierOp br : toHoist)
      br.erase();
  });
}
  ...
  mod.walk([&](func::FuncOp fn) {
    ...
    for (Operation &op : fn.getBody().front().getOperations()) {
      ...
      if (topLevelOpHasCpuSpmAccessBeforeBarrier(&op, allocation)) {
        ...
        OpBuilder b(&op);
        b.create<wafer::BarrierOp>(op.getLoc());
        return;
      }
    }
  });
...
public:
  void getDependentDialects(DialectRegistry &registry) const override {
    registry.insert<wafer::WaferDialect, func::FuncDialect, memref::MemRefDialect,
  ...
  void runOnOperation() override {
    ...
    // `wafer::BarrierOp` before any RDMA/CPU op that may read that same origin.
    mod.walk([&](func::FuncOp fn) {
      ...
      auto markWdmaWrite = [&](Operation *op) {
        Value tgt;
        if (auto w = dyn_cast<wafer::WdmaOp>(op))
          tgt = w.getTarget();
        else if (auto w = dyn_cast<wafer::Wdma4dOp>(op))
          tgt = w.getTarget();
        else if (auto w = dyn_cast<wafer::Wdma1dOp>(op))
          tgt = w.getTarget();
      ...
      for (Block &block : fn.getBody()) {
        ...
        for (Operation &op :
          ...
          if (isa<wafer::BarrierOp, wafer::AtomicBarrierInOp, wafer::AtomicBarrierOutOp>(
                  &op)) {
            clearPending();
            continue;
          }

          if (isa<wafer::WdmaOp, wafer::Wdma4dOp, wafer::Wdma1dOp>(&op)) {
            markWdmaWrite(&op);
            continue;
          }

          if (isa<wafer::RdmaOp, wafer::Rdma4dOp, wafer::Rdma1dOp>(&op)) {
            Value src;
            if (auto r = dyn_cast<wafer::RdmaOp>(&op))
              src = r.getSource();
            else if (auto r = dyn_cast<wafer::Rdma4dOp>(&op))
              src = r.getSource();
            else if (auto r = dyn_cast<wafer::Rdma1dOp>(&op))
              src = r.getSource();
