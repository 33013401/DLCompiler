...
bufferizeDestinationStyleOpInterface(RewriterBase &rewriter,
  ...
  for (OpOperand *opOperand : op.getDpsInputOperands()) {
    ...
    FailureOr<Value> buffer =
      getBuffer(rewriter, opOperand->get(), options, bufferizationState);
    if (failed(buffer))
      return failure();
  ...
  for (OpResult opResult : op->getOpResults()) {
    ...
    FailureOr<Value> resultBuffer = getBuffer(
      rewriter, opOperand->get(), options, bufferizationState);
...
struct AtomicRMWOpInterface
  ...
  LogicalResult bufferize(Operation *op, RewriterBase &rewriter,
    ...
    FailureOr<Value> valBuffer = getBuffer(
      rewriter, atomicRMWOp.getVal(), options, bufferizationState);
    if (failed(valBuffer))
      return failure();
    FailureOr<Value> outputBuffer = getBuffer(
      rewriter, atomicRMWOp.getDst(), options, bufferizationState);
...
struct AtomicCASOpInterface
  ...
  LogicalResult bufferize(Operation *op, RewriterBase &rewriter,
    ...
    FailureOr<Value> cmpBuffer = getBuffer(
      rewriter, atomicCASOp.getCmp(), options, bufferizationState);
    if (failed(cmpBuffer))
      return failure();

    FailureOr<Value> valBuffer = getBuffer(
      rewriter, atomicCASOp.getVal(), options, bufferizationState);
    if (failed(valBuffer))
      return failure();

    FailureOr<Value> outputBuffer = getBuffer(
      rewriter, atomicCASOp.getDst(), options, bufferizationState);
    if (failed(outputBuffer))
      return failure();
...
struct BitCastOpInterface
  ...
  LogicalResult bufferize(Operation *op, RewriterBase &rewriter,
    ...
    FailureOr<Value> srcBuffer = getBuffer(
      rewriter, bitcastOp.getSrc(), options, bufferizationState);
    if (failed(srcBuffer))
      return failure();
...
struct SendOpInterface
  ...
  LogicalResult bufferize(Operation *op, RewriterBase &rewriter,
    ...
    if (isa<TensorType>(sendOp.getDstAddr().getType())) {
        FailureOr<Value> dstBuffer = getBuffer(
          rewriter, sendOp.getDstAddr(), options, bufferizationState);
      if (failed(dstBuffer))
        return failure();
    ...
    if (isa<TensorType>(sendOp.getSrc().getType())) {
        FailureOr<Value> srcBuffer = getBuffer(
          rewriter, sendOp.getSrc(), options, bufferizationState);
      if (failed(srcBuffer))
        return failure();
