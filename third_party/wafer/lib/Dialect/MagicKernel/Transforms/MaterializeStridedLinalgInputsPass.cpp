  ...
  void process(func::FuncOp &func) {
    ...
    for (Operation *op : vsOps) {
      ...
      for (OpOperand &init : vsOp.getDpsInitsMutable()) {
        ...
        rewriter.setInsertionPoint(op);
        Value originalInit = init.get();
        Value alloc = createContiguousAlloc(initSubviewType, originalInit, loc);
        rewriter.create<memref::CopyOp>(loc, originalInit, alloc);
        rewriter.modifyOpInPlace(op, [&]() { init.set(alloc); });

        rewriter.setInsertionPointAfter(op);
        // init now refers to alloc; copy back to the saved original view.
        rewriter.create<memref::CopyOp>(loc, alloc, originalInit);
      }
    }
  }
