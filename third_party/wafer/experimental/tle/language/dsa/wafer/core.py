...
Exposed as ``triton.experimental.tle.language.dsa.wafer``, mirroring the
...
SPM = scope("wafer.spm", "spm", "spm")

# Fmt_INT64 in wafer tx81 Data_Format enum (see instr_def / op_def.h).
...
def randgen(seed0, seed1, n_out: tl.constexpr, _semantic=None):
    ...
    if n_out <= 0 or (n_out % _DSA_RANDGEN_NUM_STREAMS) != 0:
        raise ValueError(f"tle.dsa.wafer.randgen n_out must be a positive multiple of "
    ...
    if not seed0.dtype.is_int() or not seed1.dtype.is_int():
        raise ValueError("tle.dsa.wafer.randgen seeds must be integer tensors")
    ...
    if (not seed0_ty.is_block() or not seed1_ty.is_block()
        ...
        raise ValueError("tle.dsa.wafer.randgen seeds must be block tensors of shape [16]")

    out_ty = tl.block_type(tl.int64, [n_out])
    byte_count = n_out * 8

    rand_op = _dsa_ir.create_dsa_randgen(builder,
...
def _i64_as_i32_view(raw_i64, n_i32: int, builder):
    ...
    handle = _dsa_ir.create_dsa_bitcast(builder, dst_ty.to_ir(builder), raw_i64.handle)
...
def rand(seed0, seed1, n_out: tl.constexpr, _semantic=None):
    ...
    if n_out <= 0 or (n_out % 32) != 0:
        raise ValueError(f"tle.dsa.wafer.rand n_out must be a positive multiple of 32, got {n_out}")
...
def randn(seed0, seed1, n_out: tl.constexpr, _semantic=None):
    ...
    n_out = int(tl._unwrap_if_constexpr(n_out))
    if n_out <= 0 or (n_out % 32) != 0:
        raise ValueError(f"tle.dsa.wafer.randn n_out must be a positive multiple of 32, got {n_out}")
