...
def triton_kernel(
    ...
    x_sub = dl.extract_slice(x, [block_start + SLICE_OFFSET], [SLICE_SIZE], [1])
    y_sub = dl.extract_slice(y, [block_start + SLICE_OFFSET], [SLICE_SIZE], [1])
    output_sub = x_sub + y_sub
    output = tl.load(output_ptr + offsets, mask=mask)
    output = dl.insert_slice(
        output, output_sub, [block_start + SLICE_OFFSET], [SLICE_SIZE], [1]
    )
...
def triton_func(x: torch.Tensor, y: torch.Tensor, slice_offset: int, slice_size: int):
    ...
    triton_kernel[grid](
        x_txda, y_txda, output_txda, n_elements, BLOCK_SIZE=1024, SLICE_OFFSET=0, SLICE_SIZE=32
    )
    with torch.no_grad():
...
def test_insert_slice():
    ...
    torch.testing.assert_close(
        triton_cal[slice_offset : slice_offset + slice_size],
